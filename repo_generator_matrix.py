...
# Comment out this line if you have .pyc or .pyo files you need to keep
        ...
        self._patch_addons_matrix()
        self._generate_addons_file()
        self._generate_md5_file()
        self._unpatch_addons_matrix()
        print "Finished updating addons xml and md5 files"


    def _patch_addons_matrix(self):
        # addon list
        addons = os.listdir('.')
        ...
        for addon in addons:
            try:
                ...
                filedata = filedata.replace('name="Yatse Kodi helper script" version="', 'name="Yatse Matrix helper script" version="9')
    ...
    def _unpatch_addons_matrix(self):
        # addon list
        addons = os.listdir('.')
        ...
        for addon in addons:
            try:
                ...
                filedata = filedata.replace('name="Yatse Matrix helper script" version="9', 'name="Yatse Kodi helper script" version="')
                # Write the file out again
                with open(_path, 'w') as file:
                    file.write(filedata)
            except Exception, e:
    ...
    def Create_Zips(self,addon_id,version):
        ...
        addon_folder = os.path.join('matrix',addon_id)
        if not os.path.exists(addon_folder):
            os.makedirs(addon_folder)

        final_zip = os.path.join('matrix',addon_id,'%s-%s.zip' % (addon_id,version))
...
# clean and add closing tag
        ...
        self._save_file(addons_xml.encode( "utf-8" ), file=os.path.join('matrix','addons.xml'))

    def _generate_md5_file(self):
        try:
            m = md5.new(open(os.path.join('matrix','addons.xml')).read()).hexdigest()
            self._save_file(m, file=os.path.join('matrix','addons.xml.md5'))
        except Exception, e:
