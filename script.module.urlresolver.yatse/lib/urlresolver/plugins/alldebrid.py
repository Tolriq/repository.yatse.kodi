...
class AllDebridResolver(UrlResolver):
    ...
    def get_settings_xml(cls):
        ...
        xml.append('<setting id="%s_auth" type="action" label="%s" action="RunPlugin(plugin://script.module.urlresolver.yatse/?mode=auth_ad)"/>' % (cls.__name__, i18n('auth_my_account')))
        xml.append('<setting id="%s_reset" type="action" label="%s" action="RunPlugin(plugin://script.module.urlresolver.yatse/?mode=reset_ad)"/>' % (cls.__name__, i18n('reset_my_auth')))
