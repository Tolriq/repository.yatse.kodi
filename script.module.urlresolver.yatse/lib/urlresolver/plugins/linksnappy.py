...
class LinksnappyResolver(UrlResolver):
    ...
    def get_settings_xml(cls):
        ...
        xml.append(
            '<setting id="{0}_auth" type="action" label="{1}" action="RunPlugin(plugin://script.module.urlresolver.yatse/?mode=auth_ls)" option="close"/>'.format(
                cls.__name__, i18n('auth_my_account')
            )
        )

        xml.append(
            '<setting id="{0}_reset" type="action" label="{1}" action="RunPlugin(plugin://script.module.urlresolver.yatse/?mode=reset_ls)"/>'.format(
